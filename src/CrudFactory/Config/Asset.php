...
final class Asset
{
    ...
    /**
     * Assets provided by SyliusEasyCrudBundle (e.g. 'bundles/syliuseasycrudbundle/app.js') are versioned
     * and managed through a special Symfony Asset named package.
     * Call this method instead of '::new()' when adding those SyliusEasyCrud assets so
     * they use the right package name (which is needed later when calling to 'asset()' Twig function).
     *
     * @param string $value The 'path' when adding CSS or JS files and the 'entryName' when adding Webpack Encore entries
     */
    public static function fromEasyCrudAssetPackage(string $value): self
    {
        return self::new($value)->package(AssetEasyCrudPackage::PACKAGE_NAME);
    }
